...
def validarQuestaoVF(navegador, link, contQuestaoVF):
    # timeInicio = time.perf_counter()
    # print("Analisando as configurações da Questão Verdadeiro/Falso: %s" % navegador.find_element(by=By.ID,value="id_name").get_attribute('value'))
    checagemTotal = 0
    contQuestao = 0
    ...
    if navegador.find_element(by=By.ID, value="id_generalfeedbackeditable").text == "":
        ...
        contQuestao += 1
    # FEEDBACK PARA A OPÇÃO VERDADEIRO
    checagemTotal += 1
    input_QuestaoFeedbackVerdadeiro = False
    # print(navegador.find_element(by=By.ID,value="id_feedbacktrueeditable").text)
    if navegador.find_element(by=By.ID, value="id_feedbacktrueeditable").text == "":
        # print("Feedback verdadeiro vazio")
        input_QuestaoFeedbackVerdadeiro = True
        contQuestao += 1
    # FEEDBACK PARA A OPÇÃO FALSO
    checagemTotal += 1
    input_QuestaoFeedbackFalso = False
    # print(navegador.find_element(by=By.ID,value="id_feedbacktrueeditable").text)
    if navegador.find_element(by=By.ID, value="id_feedbackfalseeditable").text == "":
        # print("Feedback falso vazio")
        input_QuestaoFeedbackFalso = True
        contQuestao += 1
    ...
    if contQuestao > 0:
        ...
        if input_QuestaoFeedbackVerdadeiro != False:
            print("'Feedback para a opção Verdadeiro' está vazio")
        if input_QuestaoFeedbackFalso != False:
            print("'Feedback para a opção Falso' está vazio.")
    else:
